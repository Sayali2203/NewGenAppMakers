  ...
  errorHandling.config(function($provide, $httpProvider, $compileProvider) {
      ...
      var showMessage = function(content, cl,key) {
    	  $('#http-error-id').empty();
    	  $('#http-error-id').addClass(cl).appendTo(elementsList).html("<div id='errorImage' style='margin:-6px;position:absolute;' class='mcf-notification-icon mcf-notification-icon-warning'><img src='images/mcf-message-warning.png' alt='warning'></div>" +"<p style='margin-left:35px !important;margin-top:1px !important'>" + content +"</p>" );
